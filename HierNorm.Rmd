...
It takes around 3 seconds to complete the process.  

### Convergence Diagnostic

Before making parameter inference, the posterior predictive checks are employed to evaluate the fit of the model. 

```{r StanTrace, eval=TRUE, fig.width=10, fig.height=4,fig.cap='Trace plot and density estimates of parameters from MCMC samples by Stan'}
...
```{r lambda, eval=TRUE, fig.width=10, fig.height=4, fig.cap="Posterior of parameter of interest with true parameter value by Stan"}
...
The intact code snippet can be found in Section \@ref(sec:pymc3) and packages used in the are listed below:
...
Note that the `transform` argument in Inverse-Gamma variables are set to be `None` to faciliate the correctness check following.

### Correctness Check 

To check the correctness of the model in PyMC3, the log-posterior are checked against R at the same parameter values. It's been validated that the log-posterior in PyMC3 will produce the same value as in R using `logpost()`. 

```{python}
# Example Code:
logp = hnm_mod.logp
print(logp({'lambda_tilde':1, 'tau_sq':5, 'sigma_sq': 4}))
```

Details of `logp` method can be found in the [PyMC3 developer guide](https://docs.pymc.io/developer_guide.html)

### MCMC Sampling

Note that, different from the Stan, PyMC3 will assign draws and warm-up draws separately. To be consistent with the sampling in Sta, `draws` and `tune` are assigned to half of the number of samplers. 
...
It takes around 100 seconds to complete the process.

### Convergence Diagnostic 

[PyMC3 ploting](https://docs.pymc.io/api/plots.html) provides visualization tool for . Plots are delegated to the [ArviZ libaray](https://arviz-devs.github.io/arviz/). The traceplot and the posterior plot in Section \@ref(sec:pymc3inf) will be produced with ArviZ methods, while the alternatives in PyMC3 will be provided in code comment.
...
```{r, eval=TRUE, echo=FALSE, fig.width=10, fig.height=4, fig.cap='Trace plot and density estimates of parameters from MCMC samples by PyMC3'}
knitr::include_graphics('trace_pymc3.png')
```

### Parameter Inference{#sec:pymc3inf}
...
```{r, eval=TRUE, echo=FALSE, fig.width=10, fig.height=4, fig.cap='Posterior of parameter of interest with true parameter value by PyMC3: the orange line is the true parameter value along with the corresponding probability values in posterior density'}
knitr::include_graphics('hist_pymc3.png')
```

### Intact Code Snippet{#sec:pymc3}
...
This section will implement the MCMC sampling with NumPyro packages, installation and examples are provided in [Pyro project on NumPy](https://pypi.org/project/numpyro/) and [NumPyro documentation](http://num.pyro.ai/en/stable/#).

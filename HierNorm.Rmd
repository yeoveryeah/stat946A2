...
* The weakly weakly informative prior prior for $\lambda$ is adopted due to the lack of information of the population mean.
...
The the model specified in the Stan modeling language can be complied with the `stan_model()` function to construct a stanmodel object:
...
The summary statistics of chain-merged posterior samples per parameter are produced. The mean posterior estimate of lambda are close to the true value while that of the other parameter bias from the true values. 
...
[PyMC3 ploting](https://docs.pymc.io/api/plots.html) provides visualization tool for . Plots are delegated to the [ArviZ libaray](https://arviz-devs.github.io/arviz/). The trace plot and the posterior plot in this section will be produced with ArviZ methods, while the alternatives in PyMC3 will be provided in code comment.
...
The intact code snippet can be found in Section \@ref(sec:hnm-pyrocode) and packages used the are listed below:
...
A list of continuous distribution are listed on the [website](http://num.pyro.ai/en/stable/distributions.html#continuous-distributions). Note that the rate parameter in the inverse gamma distribution is actually corresponding to the scale parameter in the [literatures](https://en.wikipedia.org/wiki/Inverse-gamma_distribution).

A `sample` returns a random sample from the stochastic function specified. A `plate` context manager is constructed for annotating conditionally independent variables, the observations $y_i$'s.
...
To check the log-posterior, `log_density` method can be applied and the model are implemented correctly.
...
2. Provides access to MCMC inference algorithms in NumPyro.
